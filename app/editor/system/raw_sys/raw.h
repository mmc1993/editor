...
public:
    struct Word {
        uint mID;                   //  ◊÷Unicode±‡¬Î
        glm::vec4 mTexUV;           //  Œ∆¿Ì◊¯±Í
        glm::vec3 mRenderOffset;    //  ‰÷»æ∆´“∆
    };
public:
    GLFont();
    ~GLFont();
...
public:
    struct Pass {
        iint        mBlendSrc;          //  ªÏ∫œ
        iint        mBlendDst;          //  ªÏ∫œ
        iint        mStencilOpFail;	    //	ƒ£∞Â≤‚ ‘ ß∞‹
        iint        mStencilOpZFail;	//	…Ó∂»≤‚ ‘ ß∞‹
        iint        mStencilOpZPass;	//	…Ó∂»≤‚ ‘Õ®π˝
        iint		mStencilFunc;	    //	ƒ£∞Â≤‚ ‘∫Ø ˝
        iint        mStencilMask;       //  ƒ£∞Â≤‚ ‘÷µ
        iint        mStencilRef;        //  ƒ£∞Â≤‚ ‘÷µ
        uint        mID;                //  ID
